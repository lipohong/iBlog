<template>
    <div>
        <div v-if="author" style="display: flex; align-items: center">
            <v-avatar @click="redirectToAuthorProfile" style="cursor: pointer;" size="40" :color="secondaryColor">
            ...
            <div class="ma-2" >
                ...
                <div v-if="$store.state.authentication.userId !== author['_id']">
                    <v-btn x-small :color="primaryColor" outlined>{{ $t('pages.blog.follow') }}</v-btn>
                </div>
            </div>
        </div>
    </div>
